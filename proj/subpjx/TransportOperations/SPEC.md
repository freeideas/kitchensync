...
that peer's sync root. The public interface carries that connected peer handle
on each root-bound operation so the implementation can use the existing local
root or SSH/SFTP session instead of opening a connection. This child performs
the requested filesystem operation against the peer's winning URL only. It does
not choose fallback URLs, reconnect to another URL, or decide sync outcomes.

## Responsibilities

TransportOperations exposes `list_dir(peer, path)`. The operation lists only the
...
TransportOperations exposes `stat(peer, path)`. For an existing regular file or
...
- `open_read(peer, path)` opens an existing regular file for streaming reads.
...
- `open_write(peer, path)` opens a file for streaming writes.
- `open_write(peer, path)` creates the target file when it does not exist.
- `open_write(peer, path)` creates missing parent directories for the target
  file.
...
- `rename(peer, src, dst)` moves an entry within the same peer filesystem when
  `dst` does not already exist.
- `delete_file(peer, path)` removes a file.
- `create_dir(peer, path)` creates a directory and any missing parent
  directories.
- `delete_dir(peer, path)` removes an empty directory.
- `set_mod_time(peer, path, time)` updates the modification time of a regular
  file.
- `set_mod_time(peer, path, time)` updates the modification time of a
  directory.
...
example, a `not_found` from `stat(peer, path)` is the same absence signal for a
local peer and an SFTP peer, and an SFTP network failure is an `io_error` rather
than a separate network-specific outcome.
...
TransportOperations does not require `rename(peer, src, dst)` to overwrite an
...
connected peer handle supplied to the operation, and operations must not escape
that root by following symbolic links because symbolic links are treated as
absent or omitted.
