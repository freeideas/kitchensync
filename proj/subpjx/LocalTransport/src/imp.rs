...
impl LocalTransport for LocalTransportImpl {
    fn connect( &self, request: LocalConnectionRequest, ) -> Result<ConnectedPeerRoot, PeerTransportError> {
        unimplemented!()
    }
...
pub fn new(peertransportsurface: std::sync::Arc<dyn peertransportsurface::PeerTransportSurface>) -> std::sync::Arc<dyn LocalTransport> {
    Arc::new(LocalTransportImpl { peertransportsurface })
}
