...
fn transport_status_and_summary_are_consistent() {
    ...
    assert_eq!(status.purpose, "Local filesystem and SSH/SFTP file tree operations.");
...
fn transport_stat_reports_file_and_directory_and_not_found_for_missing_path() {
    ...
    let dir_meta = transport.stat(&RelPath::new("archive").unwrap()).expect("dir stat");
...
fn transport_create_dir_delete_file_delete_dir_operate_on_expected_paths() {
    ...
    transport.create_dir(&nested_dir).expect("create nested dir");
...
fn transport_set_mod_time_updates_entry_modification_time_for_file() {
    ...
    assert_eq!(transport.stat(&path).expect("stat after set").mod_time, stamped);
}
...
fn transport_set_mod_time_updates_entry_modification_time_for_directory() {
    ...
    assert_eq!(transport.stat(&path).expect("dir stat after set").mod_time, stamped);
}