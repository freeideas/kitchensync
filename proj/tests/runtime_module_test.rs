...
impl OrderAwareCopyOperation {
    fn new(failing_attempts_by_destination: HashMap<String, usize>, sleep_per_attempt: Duration) -> Self {
        Self {
            attempts_by_destination: Arc::new(Mutex::new(HashMap::new())),
...
impl kitchensync::runtime::CopyOperation for OrderAwareCopyOperation {
    fn execute_copy_attempt(
        ...
        {
            let mut log = self.attempt_log.lock().expect("order operation log poisoned");
...
fn mk_task(index: usize) -> CopyTask {
    CopyTask {
        ...
        destination_path: RelPath::new(format!("destination/{index}.txt")).expect("valid destination path"),
...
fn copy_scheduler_limits_inflight_attempts_to_configured_max_copies() {
    ...
    let scheduler = kitchensync::runtime::CopyScheduler::new(config, diagnostics.clone(), progress.clone());
...
fn copy_scheduler_emits_progress_events_for_copy_attempt_lifecycle() {
    ...
    assert!(events.iter().any(|event| {
        matches!(
            event,
            kitchensync::ProgressEvent::CopyStarted { .. }
        )
    }));
    assert!(events.iter().any(|event| {
        matches!(
            event,
            kitchensync::ProgressEvent::CopyProgress { .. }
        )
    }));
    assert!(events.iter().any(|event| {
        matches!(
            event,
            kitchensync::ProgressEvent::CopyFinished { .. }
        )
    }));
}
...
fn copy_scheduler_emits_copy_slot_trace_events_on_acquire_and_release() {
    ...
    assert!(slot_messages.iter().any(|message| message == "copy-slots active=1/1"));
    assert!(slot_messages.iter().any(|message| message == "copy-slots active=0/1"));
}
...
fn copy_failure_diagnostics_include_path_peer_phase_and_error() {
    ...
    assert!(messages
        .iter()
        .any(|message| message.contains("copy failed path=destination/1.txt")
...
fn stdout_output_sinks_do_not_change_scheduler_summary() {
    ...
    scheduler.submit(mk_task(0));
    scheduler.close();
    let summary = scheduler.run_until_complete(&ConcurrentCopyOperation::new(Duration::from_millis(0)));
