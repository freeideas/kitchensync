...
- **SFTP connection pool** — per user+host pools owned by the `sftp-protocol` component (two URLs sharing the same user+host share a pool), `--mc` cap, `--ka` keep-alive TTL on close, idle reuse within the TTL window
- **Edge cases** — empty directories, deep paths, timestamp tolerance (5 seconds)
- **Component boundaries** — each standalone component (see `decomposition.md`) is independently exercised at the component level; end-to-end tests drive the full kitchensync CLI
