...
2. Register OS filesystem watchers on every `file://` peer's root directory (recursive). If the OS rejects a watch (permissions, unsupported filesystem, too many watches), log a warning and continue. If no watches succeed, print an error and exit 1
