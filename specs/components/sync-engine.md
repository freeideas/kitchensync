...
The core sync algorithm: combined-tree walk, entry classification, decision rules, snapshot updates, and the operation queue.
...
def copy_file(src_peer, path, dst_peer):
    # Acquire in lexicographic URL order to prevent deadlock (see concurrency spec)
    ...
    try:
        ...
        spawn reader(src_conn, path, ch)       # reads chunks, sends to channel
        spawn writer(dst_conn, tmp_path, ch)   # receives chunks, writes to disk
        wait(reader, writer)
