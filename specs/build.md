...
| darwin  | arm64  | `kitchensync.mac`   |

`GOOS=<os> GOARCH=<arch> go build -o ../released/<binary> ./cmd/kitchensync`

## Go Module

The module is `kitchensync`. Source lives under `./code/` (where `go.mod` lives). All `go build` commands run from `./code/`; output paths in step 2/3 use `../released/` to reach the project root.
