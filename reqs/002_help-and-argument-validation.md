...
## $REQ_IDs
- `002.1` -- Running `kitchensync` with no arguments prints the fenced help screen from `specs/help.md` verbatim to stdout.
...
- `002.36` -- Non-help invocations reject `-x` when its value contains a NUL character.
- `002.37` -- Non-help invocations accept URL query parameter `timeout-conn` on peer URLs.
- `002.38` -- Non-help invocations accept URL query parameter `timeout-idle` on peer URLs.
- `002.39` -- Non-help invocations reject URL query parameter `max-copies` on peer URLs.
- `002.40` -- Non-help invocations reject any URL query parameter other than `timeout-conn` or `timeout-idle`.
- `002.41` -- URL query parameter `timeout-conn` accepts a positive integer value.
- `002.42` -- URL query parameter `timeout-idle` accepts a positive integer value.
- `002.43` -- URL query parameter `timeout-conn` rejects a value that is not a positive integer.
- `002.44` -- URL query parameter `timeout-idle` rejects a value that is not a positive integer.
- `002.45` -- A valued global option without its required value fails validation.
- `002.46` -- On any non-help validation failure, `kitchensync` prints an error message followed by the fenced help screen from `specs/help.md` to stdout.
- `002.47` -- On any non-help validation failure, `kitchensync` exits 1.
- `002.48` -- On any non-help validation failure, `kitchensync` leaves stderr empty.
