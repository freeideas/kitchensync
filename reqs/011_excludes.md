...
## $REQ_IDs
- `011.1` -- Each accepted `-x <relative-path>` value excludes its named relative path for that run.
...
- `011.10` -- Excluded paths are not scanned as peer content during the run.
- `011.11` -- Excluded directories are not recursed into as peer content during the run.
- `011.12` -- Excluded entries are not copied during the run.
- `011.13` -- Existing excluded entries are not deleted during the run.
- `011.14` -- Existing excluded entries are not displaced during the run.
- `011.15` -- Snapshot rows for excluded paths are not consulted during the run.
- `011.16` -- Snapshot rows for excluded paths are not updated during the run.
...
Metadata access under `.kitchensync/` for snapshots, SWAP recovery, and cleanup
is outside this sync-exclude category.
