...
- `012.10` -- During traversal, active subordinate peers participate in directory listing.
- `012.11` -- During decision selection, subordinate peer entries do not contribute to sync decisions.
- `012.12` -- After a sync decision is selected, subordinate peers receive the selected outcome.
- `012.13` -- A normal run uploads updated snapshot data back to subordinate peers.
- `012.14` -- A dry run does not upload updated local temporary snapshot data back to subordinate peers.
- `012.15` -- A peer that was subordinate in a previous normal run participates as a contributing peer on a later run when it is reachable, has snapshot history, and is not marked with `-`.
- `012.16` -- A peer that is unreachable at startup and does not cause startup to exit is omitted from all listings for that run.
- `012.17` -- A peer that is unreachable at startup and does not cause startup to exit is omitted from all sync decisions for that run.
- `012.18` -- A peer that is unreachable at startup and does not cause startup to exit has no snapshot rows modified during that run.
- `012.19` -- When a peer that was unreachable in one run is reachable on a later run, discrepancies between its filesystem state and its existing snapshot rows drive sync decisions.
