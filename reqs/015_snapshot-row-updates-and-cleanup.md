...
- `015.16` -- After destination directory creation succeeds, the destination peer's snapshot row for that directory records a new `last_seen` timestamp.
- `015.17` -- If destination directory creation fails, the destination peer's existing snapshot row for that directory is not changed by the failed creation.
- `015.18` -- After an entry is successfully displaced to `BAK/`, that peer's snapshot row for the displaced entry has `deleted_time` set to the row's previous `last_seen` value.
- `015.19` -- If displacement to `BAK/` fails, the affected peer's existing snapshot row for that entry is not changed by the failed displacement.
- `015.20` -- After a directory is successfully displaced to `BAK/`, non-tombstone snapshot rows in that same peer's displaced subtree get the displaced directory row's deletion estimate as `deleted_time`.
- `015.21` -- A directory displacement cascade does not change already-tombstoned rows in the displaced subtree.
- `015.22` -- A directory displacement cascade does not change snapshot rows outside the displaced subtree.
- `015.23` -- When the same directory is displaced on multiple peers, each peer's displacement cascade updates only that peer's own snapshot database.
- `015.24` -- Snapshot cleanup removes tombstone rows whose `deleted_time` is older than `--keep-del-days` days.
- `015.25` -- Sync decisions proceed without waiting for snapshot cleanup to finish in the current run.
- `015.26` -- Orphaned non-tombstone snapshot rows that cannot be reached by a directory displacement cascade are cleaned up after their `last_seen` is older than `--keep-del-days` days.
