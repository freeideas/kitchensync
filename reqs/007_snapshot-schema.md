...
- `007.13` -- Non-NULL `snapshot.mod_time`, `snapshot.last_seen`, and `snapshot.deleted_time` values use `YYYY-MM-DD_HH-mm-ss_ffffffZ` format.
...
- `007.20` -- The `snapshot.deleted_time` column has SQLite type `TEXT` and allows NULL.
- `007.21` -- A NULL `snapshot.deleted_time` value represents an entry that exists.
- `007.22` -- A non-NULL `snapshot.deleted_time` value marks the row as a tombstone.
- `007.23` -- Tombstone rows remain in the `snapshot` table.
- `007.24` -- A newly created tombstone stores the row's current `last_seen` value in `snapshot.deleted_time`.
- `007.25` -- The `snapshot` table has an index on `parent_id`.
- `007.26` -- The `snapshot` table has an index on `last_seen`.
- `007.27` -- The `snapshot` table has an index on `deleted_time`.
- `007.28` -- The `snapshot` table stores one row for each tracked path the peer has or has had.
- `007.29` -- Every snapshot row represents a path below the sync root.
- `007.30` -- The sync root directory itself has no row in `snapshot`.
